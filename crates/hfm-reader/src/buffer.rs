...
mod tests {
    ...
    fn make_raw(offset: u64, data: &str, generation: u64) -> RawChunk {
        RawChunk::new(Offset(offset), data.to_string(), generation)
    }

    fn make_processed(offset: u64, data: &str, generation: u64) -> ProcessedChunk {
        ProcessedChunk::new(Offset(offset), data.to_string(), generation)
    }
