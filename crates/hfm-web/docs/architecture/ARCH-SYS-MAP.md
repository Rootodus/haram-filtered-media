...
### Acquisition Stage - STAGE-ACQUIRE
...
### Transformation Stage - STAGE-TRANSFORM
...
### Extraction Stage - STAGE-EXTRACT
...
- Multi-Model: If multiple models are active for the same `ContentBuffer`, the MLProcessor SHALL execute each model in parallel on Tokio's blocking thread pool. Each model receives the same `InferenceTensor` and produces its own `ProcessedBuffer`.
- Output Merging: The `ProcessedBuffer` from each model SHALL be concatenated (list of instructions) into a single `ProcessedBuffer` before passing to the Renderer.

### Finalization Stage - STAGE-RENDER
...
| `Extractor` -> `Inference(s)` | `Arc<InferenceTensor>` | Parallel Model Input (one or more models) |
| `Inference` -> `Renderer` | `ProcessedBuffer` (merged) | Transformation Orders |
